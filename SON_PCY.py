...
def main(t,support,input_file,output_file):
    try:
        start=time.time()
        ...
        x = data.map(lambda x: x.split(",")).map(lambda x: (x[0],x[1])).groupByKey().map(lambda x:(x[0],set(x[1]))).filter(lambda x:len(x[1])>=int(t)).cache()
        total = x.count()
...
if __name__ =="__main__":
    ...
    main(filter_threshold,support,input_file,output_file)
